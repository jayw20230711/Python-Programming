...
show you how to use these methods instead of going through the mathematical formula.
...
import numpy as np                                # second output
from sklearn.metrics import r2_score        # for r2 value
...
relationship the polynomial regression cannot be used to predict anything.
...
use "pip3 install -U scikit-learn scipy matplotlib" to install if missing sklearn error occurs.
...
print(r2_score(y, mymodel(x)))          # 0.9432150416451026
...
mymodel = np.poly1d(np.polyfit(x, y, 3))           # polyfit --> degree 3 return 4 points
# that is a degree 3 equation with 4 points -->  ax3 + bx2 + cx + d

myline = np.linspace(2, 95, 100)            # get x values to apply to the equation
...
"""
print(r2_score(y, mymodel(x)))          # 0.009952707566680652
